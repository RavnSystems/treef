...
object EnsembleBuilder {
  ...
  val rootField : Field =
    classOf[RegressionTree].getDeclaredField("root")
  rootField.setAccessible(true)

  val thresholdField : Field =
    classOf[LibSplit].getDeclaredField("threshold")
  thresholdField.setAccessible(true)

  val featureField : Field =
    classOf[LibSplit].getDeclaredField("featureID")
  featureField.setAccessible(true)

  val avgLabelField : Field =
    classOf[LibSplit].getDeclaredField("avgLabel")
  avgLabelField.setAccessible(true)

  def buildTreeNode(split : LibSplit,
                    features : Map[Int, Feature]) : TreeNode = {
  ...
  def buildTree(t : RegressionTree, weight : Double,
                features : Map[Int, Feature]) : Tree = {
    val rootSplit = rootField.get(t).asInstanceOf[LibSplit]
    val root = buildTreeNode(rootSplit, features)
    new Tree(weight, root)
  }

  def buildEnsemble(ensemble : LibEnsemble,
                    features : Map[Int, Feature]) : Ensemble = {
